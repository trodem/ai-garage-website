...
**Auth / post-signup surfaces:** only **`/auth/callback`**, gated in **middleware** (invalid query → redirect home) and **client** (hash/query must match `lib/authCallbackAccess.ts`). **`/[locale]/welcome` → redirect** to locale home. Valid `/auth/callback` requests must **`NextResponse.next()`** — do not pass them through `next-intl` middleware (`localePrefix: always` otherwise 404s the route). Callback outcome views are **informational only** (no store/download CTAs) **except password recovery** (`type=recovery`): that flow shows a new-password form, then success copy directing the user to open the app and sign in (**ADR-053**). Logic lives in `middleware.ts` + `AuthCallbackPageClient.tsx` + `AuthCallbackRecoveryView.tsx` — keep in sync when adding flows.
