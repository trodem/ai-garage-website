import { VehicleCessionInvitationEmail } from '@/emails/VehicleCessionInvitationEmail';
...
const bodySchema = z.object({
  to: z.string().email(),
  vehicleLabel: z.string().min(1).max(200),
...
export async function POST(request: Request) {
  ...
  const { to, vehicleLabel, inviterDisplayName, ctaUrl } = parsed.data;

  const html = await render(
    VehicleCessionInvitationEmail({
      inviteeEmail: to,
      vehicleLabel,
      inviterDisplayName,
      ctaUrl,
  ...
  const subject = `Fahrtenbuch-Übertragung für „${vehicleLabel}“`;
  ...
  if (error) {
    console.error('resend_send_vehicle_cession_invitation_failed', {
